...
- **Correção de Sincronização APK↔Web (Firebase)**: Corrigidos múltiplos problemas que impediam a sincronização em tempo real entre o APK nativo (Capacitor) e a versão web (Vercel). (1) `enableIndexedDbPersistence` foi isolado para executar somente quando `Capacitor.isNativePlatform() === true` — na web, o cache local não bloqueia mais snapshots externos. (2) O `useSyncedStorage` foi refatorado para usar `useRef` no timestamp do último save, eliminando re-registros desnecessários do listener a cada escrita. (3) O `onSnapshot` agora usa `{ includeMetadataChanges: false }`, garantindo recepção do servidor e não do cache. (4) Criado o hook `useFirestoreStatus.ts` que observa eventos `online/offline` do navegador e expõe um estado global de sincronização (`synced`, `syncing`, `offline`). (5) Adicionado indicador visual discreto de sync no header do `Dashboard` (ícone de nuvem com 3 estados + ícone giratório durante gravação). (6) Regras do Firestore atualizadas para incluir o `profileId` `rosimere`. (Atualizado em: 24/06/2026 09:20 - Arquivos: config.ts, useSyncedStorage.ts, useFirestoreStatus.ts, Dashboard.tsx, firestore.rules)
## 12. DÉBITOS TÉCNICOS
