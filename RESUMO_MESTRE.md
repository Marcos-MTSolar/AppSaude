...
- **Adição do Perfil Rosimere (Cardápio Oncológico)**: Foi adicionado um terceiro perfil no app ("Rosimere") com foco exclusivo em suporte e recuperação nutricional (quimioterapia). A interface principal do `Dashboard` foi isolada: para este perfil, apenas a aba "Cardápio" é exibida. Na aba de cardápio, a lógica de dia automático foi substituída por um seletor manual de "Fases" (ex: "Dia da Quimio", "Semana de Descanso"), e dicas específicas para cada refeição foram injetadas para facilitar o dia a dia. Também foi implementado um banner fixo de alerta médico e o tema principal foi ajustado para tons Rose. (Atualizado em: 24/06/2026 09:00 - Arquivos: profiles.ts, meals.ts, ProfileSelector.tsx, Dashboard.tsx, CardapioTab.tsx, theme.ts)
- **Visualização do Cardápio de Amanhã**: Implementado um seletor "Hoje / Amanhã" na aba de Cardápio (exclusivo para Marcos e Sandra). Quando selecionado "Amanhã", o sistema calcula o cardápio do próximo dia do plano e exibe uma tag visual indicativa. Para evitar preenchimento acidental, os checkboxes de conclusão de refeição são removidos da interface durante a visualização do dia futuro. (Atualizado em: 24/06/2026 09:10 - Arquivos: CardapioTab.tsx)
- **Opção de Almoço "Em Casa" (Perfil Marcos)**: Adicionada a propriedade `opcaoCasa` na estrutura de dados do cardápio (`meals.ts`) especificamente para o almoço do Marcos nos dias programados como "refeição de trabalho". Na aba de Cardápio, um botão de toggle "🏠 Estou em casa hoje" permite ao usuário alternar a visualização para um cardápio caseiro, prático e focado na culinária nordestina (composto de arroz, feijão, proteína, legume e/ou opções como cuscuz com ovo e macaxeira com carne de sol). A preferência fica persistida no `localStorage` por dia de ciclo (`marcos_almoco_em_casa_{dataDayIndex}`). (Atualizado em: 24/06/2026 09:15 - Arquivos: meals.ts, CardapioTab.tsx)
- **Correção de Sincronização APK↔Web (Firebase)**: Corrigidos múltiplos problemas que impediam a sincronização em tempo real entre o APK nativo (Capacitor) e a versão web (Vercel). (1) `enableIndexedDbPersistence` foi isolado para executar somente quando `Capacitor.isNativePlatform() === true` — na web, o cache local não bloqueia mais snapshots externos. (2) O `useSyncedStorage` foi refatorado com `useRef` para timestamp (elimina re-registro de listener). (3) `onSnapshot` com `includeMetadataChanges:false`. (4) Novo hook `useFirestoreStatus` com indicador visual no `Dashboard`. (5) `firestore.rules` atualizado para incluir `rosimere`. (6) Criados `firebase.json` e `firestore.indexes.json` para habilitar deploy via CLI. (Atualizado em: 24/06/2026 09:20 - Arquivos: config.ts, useSyncedStorage.ts, useFirestoreStatus.ts, Dashboard.tsx, firestore.rules, firebase.json, firestore.indexes.json)
- **Correções de Sincronização v3 (AcompanhamentoTab + useSyncedStorage)**: (1) `weights` em `AcompanhamentoTab` migrado de `useLocalStorage` para `useSyncedStorage` — histórico de pesagem agora sincroniza entre APK e web. Import de `useLocalStorage` removido do arquivo. (2) Prefixo duplicado do `profileId` no caminho do Firestore corrigido: a chave do localStorage permanece completa (ex: `marcos_weights`) para retrocompatibilidade, mas o `useSyncedStorage` agora calcula `firestoreKey` removendo o prefixo, resultando no caminho correto `usuarios/marcos/dados/weights`. O mesmo ajuste foi propagado para a chamada do `saveProfileData`. (3) Erro no callback do `onSnapshot` agora chama `notifySyncing(false)`, tornando o indicador visual do header ciente de falhas de conexão em tempo real. Regras do Firestore republicadas via `firebase deploy --only firestore:rules --project appsaude-7ceb3`. (Atualizado em: 24/06/2026 09:35 - Arquivos: AcompanhamentoTab.tsx, useSyncedStorage.ts, firebase.json, firestore.indexes.json)
- **Correções Definitivas APK (Polling e Tratamento de Erros)**: O `onSnapshot` (via gRPC) estava sendo bloqueado no WebView do Capacitor no Android. Para contornar, o `useSyncedStorage` agora usa estratégia híbrida: na Web (Vercel) usa `onSnapshot`, mas no APK nativo (`Capacitor.isNativePlatform()`) usa um _polling_ passivo via `getDoc` a cada 10 segundos. O `syncService.ts` foi atualizado com blocos `try/catch` para disparar um `throw error` quando o `setDoc` falha, permitindo que a falha bolhe até o `useSyncedStorage` (que avisa a interface). O `config.ts` recebeu um log de inicialização explícito informando a plataforma. (Atualizado em: 24/06/2026 09:50 - Arquivos: syncService.ts, useSyncedStorage.ts, config.ts)
## 12. DÉBITOS TÉCNICOS
